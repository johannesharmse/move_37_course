# adapted from https://github.com/AndersonJo/dqn-pytorch/blob/master/dqn.py
...
parser.add_argument('--skip_action', default=int(4/4), type=int, help='Skipping actions')
...
class Agent(object):
    ...
    def select_action(self, states: np.array) -> tuple:
        ...
        if self.mode == 'dqn':
            ...
            # print(self.dqn(states_variable).data.cpu().max(1)[1].unsqueeze(1))
    ...
    def optimize(self, gamma: float):
        ...
        # print(Variable(torch.cat(transitions.action).cuda()).size())
        ...
        # print(reward_batch[non_final_mask])
        # print(target_pred.max(1)[0] * gamma)
        # print(target_values[non_final_mask])
        # print(gamma)
        # print(BATCH_SIZE)
        test = reward_batch[non_final_mask] + target_pred.max(1)[0] * gamma
        test = test.unsqueeze(1)
        # print(test.size())
        # print(target_values[non_final_mask].size())
        target_values[non_final_mask] = test
        test2 = reward_batch[final_mask]
        # print(reward_batch[final_mask])
        # print(reward_batch)
        ...
        # print(torch.sum(reward_batch).data.cpu().numpy())
    ...
    def play(self, logging=True, human=True):
        ...
        while True:
            ...
            # print(dqn_pred.data.cpu().max(1)[1][0])
